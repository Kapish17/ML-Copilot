 ...
 *
 * Also where the Dashboard's upload and results live now, via
 * `DashboardStateProvider`. `AppShell` is rendered once by the root layout
 * and never unmounted by navigating between routes, so holding that state
 * here — rather than in `DashboardPage` itself — is what lets an uploaded
 * file survive a trip to another tab and back. See `lib/state/dashboard-state`
 * for why that trip used to lose it.
 */
